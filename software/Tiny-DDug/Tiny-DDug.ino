    ...
    while (1) {
        ...
        switch (GD_DDUG.Skip_Frame) {
            case (0):ADJUST_FRAME_MAIN_DDUG(); Tiny_Flip_TDDUG(1,0,109,0,7);
                ...
                if (GD_DDUG.FirsT_Time==1) {DLAY(); GD_DDUG.FirsT_Time=0;} if (GD_DDUG.DEAD==6) {
                    DLAY();
                    if (GD_DDUG.LIVE>0) {GD_DDUG.LIVE--; goto RELOAD_LEVEL;} else {goto RESTART;}
                } break;
...
int8_t TrackX_TDDUG(int8_t t_)
{
    if (ENEMY_DDUG[t_].TRACKING()==1) {
        ...
        if (RecupeDecalageY_TDDUG(ENEMY_DDUG[t_].X()-20)==0) {
            ...
            if ((ENEMY_DDUG[t_].X())>((GD_DDUG.GO_OUT==0)?DUG_TDDUG.X():20)) {return -1;}
            return 0;
        } else {return ENEMY_DDUG[t_].DIRECTION_X();}
    }
}
