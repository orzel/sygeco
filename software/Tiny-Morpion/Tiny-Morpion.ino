        ...
        if ((Byte_TMP&Byte_Comp)==(Byte_TMP)) {
            ...
            for (t_2=0; t_2<8; t_2++) {
                if ((Byte_TMP2&(1<<t_2))!=0) {
                    if (p[RECUPE_POS_GRID_TMORPION(t_2)]==2) {p[RECUPE_POS_GRID_TMORPION(t_2)]=Players[0]; return 1;}
                }
            }
        }
    }
...
void Check_WIN_TMORPION(uint8_t O_X)
{
    ...
    for (x_=0; x_<8; x_++) {
        ...
        if (Byte_TMP==pgm_read_byte(&LINE_CHECK_TMORPION[x_])) {
            switch (x_) {
                case 0 ... 3:if (p[4]==O_X) {if (COMPLETED_PROCEDURE_TMORPION()) {goto GO_OUT;}} break;
...
void Tiny_Flip_TMORPION(uint8_t POINTER_,uint8_t START_,uint8_t END_)
{
    ...
    for (y = START_; y < END_; y++) {
        ...
        for (x = 0; x < 128; x++) {
            SSD1306.ssd1306_send_byte(Recupe_TMORPION(x,y));
        }
...
void Tiny_MENU_TMORPION(uint8_t POINTER_)
{
    ...
    for (y = 0; y < 8; y++) {
        ...
        SSD1306.ssd1306_send_data_start();
        for (x = 0; x < 128; x++) {
            SSD1306.ssd1306_send_byte(Recupe_MENU_TMORPION(x,y)|
