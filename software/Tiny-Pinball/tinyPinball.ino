...
#define BACK (pgm_read_byte(&start[serialcount])&PixelAsign(Y))!=0b00000000)
    ...
    } else if ((((pgm_read_byte(&FLIPDETGAUCHE[(X-4)]))&(PixelAsign(Y)))!=0b00000000)||(BACK) {
        if (trigFlipG!=3) {
                B->SIMSpeedx=2;
                Sound(20,12);
    ...
    } else if ((((pgm_read_byte(&FLIPDETDROIT[(X-4)]))&(PixelAsign(Y)))!=0b00000000)||(BACK) {
        if (trigFlipD!=3) {
                B->SIMSpeedx=2;
                Sound(20,12);
            } else {B->SIMSpeedx=1; Sound(1,6);} return 0;
...
uint8_t RecupeFlip(uint8_t x,uint8_t y)
{
    ...
    if ((y==5)&&(x>35&&x<42)) {
        ...
        return pgm_read_byte(&FLIPDROITE[(x-36)+(TRIDD*6)]);
    } else {
        return 0x00;
    }
}
