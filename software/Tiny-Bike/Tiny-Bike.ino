...
NEW_START:;
    ...
    while (1) {
        ...
        for (t=0; t<CHECK_SPEED_ADJ(ACCEL); t++) {
            INCREMENTE_SCROLL(); if (DIV1==3) {
                digitalWrite(4,HIGH); _delay_us(10); digitalWrite(4,LOW); TRACK_RUN_ADJ();
...
void INCREMENTE_SCROLL(void)
{
    if (VAR_SCROLL3==0) {
        ...
        if (VAR_SCROLL2==0) {
            VAR_SCROLL2=1;
            if (VAR_SCROLL1==0) {VAR_SCROLL1=1;} else {VAR_SCROLL1=0;}
        } else {VAR_SCROLL2=0;}
    } else {VAR_SCROLL3=0;}
    if (VAR_SCROLL3==1) {
        VAR_SCROLL3=2; if (Progress_bar_Interval_timer<Progress_bar_Interval) {Progress_bar_Interval_timer++;} else {Progress_bar_Interval_timer=0; if (Progress_bar_value<32) {Progress_bar_value++;}}
