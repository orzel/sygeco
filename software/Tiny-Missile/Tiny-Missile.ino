...
uint8_t Score_Dome_Munition_TMISSILE(void)
{
    _delay_ms(500);
    while (1) {
        if (ARM_TMISSILE.USE_WEAPON()) {Scores_TMISSILE++; COMPIL_SCO_TMISSILE(); SNDBOX_TMISSILE(3); Tiny_Flip_TMISSILE(0,1,126);} else {break;}
    }
    _delay_ms(500);
    uint8_t Count_=0;
    for (uint8_t t=0; t<NUM_of_DOME_TMISSILE; t++) {
...
void Tiny_Flip_TMISSILE(uint8_t ST_,uint8_t ED_,uint8_t XS_)
{
    ...
    for (y = ST_; y < ED_; y++) {
        ...
        for (x = 0; x < XS_; x++) {
            SSD1306.ssd1306_send_byte(Recupe_TMISSILE(x,y));
        }
...
void INTRO_TMISSILE(uint8_t FL_)
{
    ...
    for (y = 0; y < 8; y++) {
        ...
        SSD1306.ssd1306_send_data_start();
        for (x = 0; x < 128; x++) {
            SSD1306.ssd1306_send_byte(RECUPE_INTRO_TMISSILE(x,y,FL_)|((y==7)?NUMERIC_TMISSILE(x):0x00));
        }
        SSD1306.ssd1306_send_data_stop();
    }
}
