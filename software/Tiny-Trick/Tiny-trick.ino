...
#define Xt1 (sprite[2].Xpos)
    if (Drag_Puck==255) {
        if (Xt1<64) {
            if ((Xt1+2)<=(sprite[3].Xpos+1)&&(Goal_0==0)) {GOAL_=1; Points_CPU++;}
        } else {
...
{
    if (xPASS>63) {return pgm_read_byte(&patinoire1_2[(127-xPASS)+(yPASS*64)]);}
...
void Tiny_Flip(void)
{
    ...
    for (y = 0; y < 8; y++) {
        ...
        for (x = 0; x < 128; x++) {SSD1306.ssd1306_send_byte(Recupe(x,y));}
...
void intro(uint8_t GOAL_SCREEN,uint8_t SLIDE)
{
    ...
    for (y = 0; y < 8; y++) {
        ...
        for (x = 0; x < 128; x++) {
            switch (GOAL_SCREEN) {
                case 0:SSD1306.ssd1306_send_byte((pgm_read_byte(&Intro[(x)+(y*128)])|blitzSprite(40,39,x,y,0,cadre40_37))); break;
                case 1:SSD1306.ssd1306_send_byte(((Recupe(x,y)|(blitzSprite(40,39+SLIDE,x,y,0,screen)))&(0xff-blitzSprite(40,39+SLIDE,x,y,1,screen)))|(blitzSprite(40+(13),39+SLIDE+(13),x,y,Points_Pl1,NUM)|blitzSprite(40+(33),39+SLIDE+(13),x,y,Points_CPU,NUM))); break;
                case 2:SSD1306.ssd1306_send_byte(Lost_win(x,y,win)); break;
                case 3:SSD1306.ssd1306_send_byte(Lost_win(x,y,lost)); break;
                case 4:SSD1306.ssd1306_send_byte(pgm_read_byte(&Intro[(x)+(y*128)])); break;
                default:break;
            }
        }
        SSD1306.ssd1306_send_data_stop();
